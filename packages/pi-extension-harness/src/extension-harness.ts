 ...
 * their own mock.
 *
 * Originally authored against a local pi-mono fork's own internal test
 * utilities (not published by @earendil-works/pi-coding-agent), then vendored
 * into one project's own test suite. This package is the canonical, shared
 * home -- every Pi extension project depends on this instead of re-vendoring
 * or hand-rolling its own fake ExtensionAPI.
 *
 * Usage:
 *
 ...
 * or any other package on the native-modules list).
 *
...
export function createExtensionHarness(factory: ExtensionFactory, options: ExtensionHarnessOptions = {}): ExtensionHarness {
	...
	function installLeakDetectors(): void {
		...
		// Also intercept console.* — test runners (bun:test, vitest, jest) replace
		// the console object before process.stdout.write, so console.log does NOT
